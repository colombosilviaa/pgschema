...
def convert_internal_representation_to_pgschema_dict(graph: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for n in nodes:
        ...
        if not orig_var or orig_var == caption:
            id_to_typename[node_id] = f"{caption}Type"
        else:
...
def _format_associations(relationships: list, nodes: list, id_to_typename: dict) -> list:
    ...
    for rel in relationships:
        ...
        if rel_type_name not in grouped_assocs:
            grouped_assocs[rel_type_name] = {
        ...
        if current_props:
            grouped_assocs[rel_type_name]["properties"].update(current_props)
        
        grouped_assocs[rel_type_name]["sources"].add(source_var)
        grouped_assocs[rel_type_name]["targets"].add(target_var)

    for rel_type_name, data in grouped_assocs.items():
        ...
        sources_str = "|".join(list(data["sources"]))
        targets_str = "|".join(list(data["targets"]))
        props_str = f" {{{', '.join(primitive_props)}}}" if primitive_props else ""
        
        formatted_rels.append(f"(:{sources_str})-[{data['rel_typename']}: {rel_label}{props_str}]->(:{targets_str})")
